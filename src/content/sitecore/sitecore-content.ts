...
6. Apply the safest fix with evidence`, relatedTopics: ['sitecore-publishing', 'sitecore-caching', 'sitecore-content-delivery'] },
  { slug: 'sitecore-headless-vs-mvc', title: 'Headless vs MVC', description: 'Understand how classic Sitecore MVC and modern headless delivery differ across authoring, rendering, and operations.', focus: 'Sitecore MVC renders the experience inside the Sitecore web application, while headless delivery moves rendering into an external frontend such as Next.js and uses Sitecore mainly for content, layout data, and authoring workflows. The decision changes deployment, preview, caching, and ownership boundaries.', why: 'Teams need this distinction early because it affects hiring, infrastructure, authoring expectations, release sequencing, and how production incidents are diagnosed. A weak decision here creates years of friction.', usage: 'Architects compare MVC and headless when evaluating legacy support, XM Cloud readiness, frontend team skills, multisite needs, composable integration goals, and how much decoupling the organization can support operationally.', workflow: 'The team compares authoring needs, personalization depth, frontend ownership, deployment model, preview reliability, and long-term product direction. Then it chooses the safer delivery model for the current organization rather than the trendiest option.', exampleTitle: 'Delivery-model comparison', exampleCode: `Choose MVC when
- Sitecore owns page rendering end to end
- the team supports an established classic implementation

Choose headless when
- a frontend team owns rendering
- composable integrations and modern deployment patterns matter
- XM Cloud or external rendering-host strategy is part of the roadmap`, relatedTopics: ['sitecore-mvc-basics', 'sitecore-headless-services', 'sitecore-nextjs-rendering-host'] },
  { slug: 'sitecore-multisite-architecture', title: 'Multisite Architecture', description: 'Learn how Sitecore multisite architecture should balance reuse, governance, localization, and team ownership.', focus: 'Multisite architecture in Sitecore is the discipline of deciding how multiple brands, regions, business units, or campaign sites share templates, components, content structure, media, and operational governance without creating one tangled platform.', why: 'Multisite decisions are high impact because they affect content-tree clarity, author permissions, deployment safety, design-system reuse, localization strategy, and how easily teams can evolve one site without breaking another.', usage: 'Enterprise programs use multisite architecture for global brand families, regional sites, campaign estates, franchise models, or multiple business units sharing one Sitecore platform with controlled variation.', workflow: 'Architects define site boundaries, shared and site-specific templates, component reuse rules, content-tree conventions, localization behavior, and ownership boundaries. Then they validate the model through real authoring and release workflows instead of only diagrams.', exampleTitle: 'Multisite governance frame', exampleCode: `Shared
- base templates
- global navigation patterns
- reusable components

Site specific
- regional content trees
- localized settings
- brand-level datasource ownership`, relatedTopics: ['sitecore-content-tree', 'sitecore-template-design', 'sitecore-environment-strategy'] },
  { slug: 'sitecore-sxa', title: 'Sitecore SXA', description: 'Understand how Sitecore SXA accelerates site building through reusable components, site setup conventions, and authoring-oriented configuration.', focus: 'Sitecore SXA, or Sitecore Experience Accelerator, provides prebuilt site-building patterns, renderings, themes, and conventions so teams can launch Sitecore sites faster without rebuilding every authoring and presentation capability from scratch.', why: 'SXA matters because it changes how much custom engineering a Sitecore team should do. It can speed delivery significantly, but the wrong customization approach can make upgrades, performance, and authoring behavior harder later.', usage: 'Teams use SXA for multisite marketing platforms, shared component libraries, accelerated site launches, and authoring-heavy implementations where standardized patterns are more valuable than fully bespoke presentation logic.', workflow: 'The team evaluates whether SXA conventions fit the implementation, configures tenants and sites, maps reusable components and datasources into the SXA model, and decides where to stay within accelerator patterns versus where custom development is justified.', exampleTitle: 'SXA decision heuristic', exampleCode: `Use SXA when
- multiple marketing sites need shared launch patterns
- authoring speed matters
- standard components cover most needs

Be cautious when
- custom rendering behavior fights SXA conventions
- teams customize the accelerator everywhere instead of governing extension points`, relatedTopics: ['sitecore-components', 'sitecore-multisite-architecture', 'sitecore-headless-vs-mvc'] },
  { slug: 'sitecore-experience-editor-vs-pages', title: 'Experience Editor vs Pages', description: 'Compare Experience Editor and Sitecore Pages so teams know how authoring expectations change across classic and modern Sitecore delivery models.', focus: 'Experience Editor belongs to the classic visual authoring model, while Sitecore Pages is part of the more modern XM Cloud and headless-oriented authoring direction. Both support page composition, but they differ in rendering-host dependency, preview assumptions, and the surrounding platform model.', why: 'Teams need this comparison because authors, developers, and stakeholders often assume both authoring modes behave the same. In practice, the preview contract, component behavior, and support workflow can differ a lot.', usage: 'This comparison appears during XM Cloud migration planning, author training, hybrid platform support, and production debugging when editors say one authoring surface behaves differently from another.', workflow: 'Engineers compare the current rendering approach, author needs, preview requirements, and migration roadmap. Then they define which authoring mode is primary, what preview guarantees exist, and how component contracts must behave in each mode.', exampleTitle: 'Authoring-mode comparison', exampleCode: `Experience Editor
- classic in-context editing
- tightly tied to traditional Sitecore presentation behavior

Pages
- modern route composition
- stronger dependence on stable headless rendering-host preview
- common in XM Cloud-oriented delivery`, relatedTopics: ['sitecore-experience-editor', 'sitecore-pages', 'sitecore-nextjs-rendering-host'] },
  { slug: 'sitecore-forms', title: 'Sitecore Forms', description: 'Learn how Sitecore Forms fits authoring workflows, lead capture, validation, and downstream integrations.', focus: 'Sitecore Forms provides an author-friendly way to build and manage forms inside the Sitecore platform, but the real engineering challenge is not only field configuration. It is validation, security, analytics, data handling, integrations, and release-safe behavior across environments.', why: 'Forms matter because they are often business critical. Weak form implementation creates broken lead capture, spam, compliance risk, unreliable authoring, and emergency production support during campaigns.', usage: 'Teams use Sitecore Forms for contact flows, brochure downloads, gated assets, campaign lead capture, event registration, and operational workflows that connect Sitecore to CRMs or marketing automation systems.', workflow: 'Authors configure the form structure, developers define validation and integration behavior, security and spam protections are added, downstream systems are connected, and the whole submission path is tested across authoring, preview, and production environments.', exampleTitle: 'Form delivery contract', exampleCode: `Author configures form
  -> visitor submits validated data
  -> Sitecore stores or forwards submission
  -> CRM / automation integration receives payload
  -> monitoring confirms successful delivery`, relatedTopics: ['sitecore-workflows', 'sitecore-external-system-integration', 'sitecore-security-roles'] },
  { slug: 'sitecore-search-solr', title: 'Search and Solr', description: 'Understand how Sitecore search and Solr-style indexing decisions affect discovery, relevance, and production support.', focus: 'In many Sitecore programs, search is not only a frontend feature. It depends on indexing strategy, content-model consistency, publish timing, query behavior, and operational visibility. Solr and similar search layers become part of the platform contract, not an isolated tool.', why: 'Search matters because broken indexes, weak schema discipline, and delayed refresh paths quickly become user-facing issues. Enterprise teams need engineers who understand how content architecture and indexing behavior connect.', usage: 'Programs use Sitecore search with Solr-like indexing for onsite search, filtered content discovery, documentation hubs, campaign content, and content experiences where freshness and findability directly affect business outcomes.', workflow: 'Content is modeled and published, indexing rules determine what enters search, the index refresh path is monitored, and the consuming frontend or search UI validates relevance, filtering, and freshness against real authoring behavior.', exampleTitle: 'Search indexing checkpoints', exampleCode: `Content model
  -> publish event
  -> index update
  -> search query
  -> filtered result UI
...
Support questions
- was the item published?
- did the index refresh?
- is the query or filter contract still correct?`, relatedTopics: ['sitecore-search-integration', 'sitecore-content-delivery', 'sitecore-performance'] },
  { slug: 'sitecore-language-fallback', title: 'Language Fallback and Localization', description: 'Learn how Sitecore language strategy, fallback behavior, and localized authoring affect multisite delivery.', focus: 'Language fallback in Sitecore determines what content is shown when a locale-specific version is missing. It sits at the center of multilingual delivery because it affects route correctness, author expectations, translation readiness, and what end users actually see in regional experiences.', why: 'Localization mistakes are expensive because they often look acceptable until a market launch exposes the wrong language, wrong version, or incomplete content. Teams need explicit fallback rules instead of accidental behavior.', usage: 'Global Sitecore programs use language fallback for regional sites, shared content models, staged translation rollouts, and launches where some locale content is ready before others.', workflow: 'Architects define language structure, which fields or items localize, what fallback behavior is allowed, and how publishing and QA verify regional completeness. Frontend and Sitecore teams then align routing and cache behavior with that model.', exampleTitle: 'Localization decision path', exampleCode: `Localized route
  -> localized item version exists?
  -> yes: serve local version
  -> no: apply approved fallback or block launch

Never rely on fallback without validating SEO, route, and author expectations`, relatedTopics: ['sitecore-multisite-architecture', 'sitecore-versioning', 'sitecore-publishing'] },
  { slug: 'sitecore-serialization-strategy', title: 'Serialization Strategy', description: 'Understand how Sitecore serialization strategy should separate source-controlled artifacts from environment-specific or editorial content.', focus: 'Serialization strategy is the governance around what should be serialized, how modules are grouped, how dependencies are reviewed, and what should remain editorial or environment-managed rather than treated like code. It is broader than simply turning serialization on.', why: 'Teams need a serialization strategy because uncontrolled serialization creates noisy repos, environment drift, broken releases, and confusion about which changes belong in pull requests versus publishing workflows.', usage: 'Strong Sitecore teams use serialization strategy to govern templates, renderings, settings, branch templates, placeholder rules, and selected platform artifacts while keeping live editorial content and environment secrets out of the wrong promotion path.', workflow: 'Architects define the serialized scope, teams map artifacts to modules, developers review changes like code, CI/CD promotes only the right artifacts, and release checks ensure content and rendering-host dependencies stay aligned.', exampleTitle: 'Serialization boundary model', exampleCode: `Serialize
- templates
- renderings
- settings
- placeholder and component definitions

Do not serialize casually
- market-by-market editorial content
- environment-specific secrets
- operational data that belongs outside source control`, relatedTopics: ['sitecore-content-serialization', 'sitecore-cli', 'sitecore-deployment-flow'] },
  { slug: 'sitecore-xm-cloud-deployment-model', title: 'XM Cloud Deployment Model', description: 'Understand how XM Cloud changes Sitecore deployment, preview, rendering-host ownership, and release sequencing.', focus: 'XM Cloud shifts Sitecore delivery toward SaaS-managed authoring and headless-oriented rendering models. That changes what the Sitecore team owns directly, how frontend deployments are coordinated, and how preview, publishing, and route validation should be handled.', why: 'This matters because teams often treat XM Cloud as only a product upgrade. In reality, it changes operational responsibilities, environment assumptions, and the contract between Sitecore, the rendering host, and release governance.', usage: 'Architects and senior developers discuss XM Cloud deployment when planning migrations from XP or XM, modernizing marketing platforms, or defining release processes for Next.js-based Sitecore frontends.', workflow: 'The team defines environment roles, integrates the rendering host, aligns preview and publish flow, treats route verification as part of release, and documents what belongs to Sitecore SaaS operations versus customer-owned frontend delivery.', exampleTitle: 'XM Cloud release model', exampleCode: `Sitecore SaaS authoring
  -> publish into delivery path
  -> rendering host deployment or revalidation
  -> route and preview verification
  -> production smoke checks

Key question: which layer owns the current user-facing failure?`, relatedTopics: ['sitecore-xm-vs-xp-vs-xm-cloud', 'sitecore-nextjs-rendering-host', 'sitecore-deployment-flow'] },
